...
import OpportunityAutopilotClient from "@/components/dashboard/OpportunityAutopilotClient";

export const metadata = {
  title: "AI Opportunity Autopilot - SRM Opportunity Intelligence Platform",
  description: "Netflix/Spotify-style opportunity recommendation cockpit. Evaluate match parameters, deadline risk indexes, and success probabilities.",
};

export default function StudentAutopilotPage() {
  return (
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-mono bg-zinc-900 border border-zinc-800 text-zinc-300">
              ...
              <span className="text-zinc-500 text-[10px]">Real-Time Active Profiling</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-zinc-100">
              AI Opportunity Autopilot
            </h1>
            <p className="text-xs sm:text-sm text-zinc-400 font-light leading-relaxed max-w-xl">
              Proactively discover jobs, internships, hackathons, and certifications. Track selection probability indices, deadline warning risks, and missed opportunities.
            </p>
          </div>
        </div>
      </div>

      {/* ── MAIN AUTOPILOT WORKSPACE ── */}
      <OpportunityAutopilotClient />
    </div>
  );
}
